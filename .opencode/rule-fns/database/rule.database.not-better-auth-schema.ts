...
export const ruleNotBetterAuthSchema: TRuleFn = async ({directory, filePath}) => {
    // Only check files in the database directory
    const normalizedPath = filePath.replace(/\\/g, "/");
    if (!normalizedPath.includes("/database/")) {
        return undefined;
    }
    ...
    if (isBetterAuthSchema(filePath)) {
        ...
        return {
            error:
                ...
                `To modify better-auth schema, edit auth.${dbName}.ts and run migrations manually:\n\n` +
                "```bash\n" +
