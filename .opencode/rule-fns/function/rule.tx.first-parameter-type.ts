...
export const ruleTxFirstParameterType: TRuleFn = async ({content, filePath}) => {
    ...
    if (parameters.length === 0) {
        return {
            error: "Transactional function must have 2 parameters. " +
    ...
    if (!firstParamType.includes("Portal")) {
        return {
            error: "Transactional function first parameter must be a Portal type (contains 'Portal'). " +
