...
export const ruleFxReturnType: TRuleFn = async ({content, filePath}) => {
    ...
    if (!returnType) {
        return {
            error: "Effectful function must have an explicit return type. " +
    ...
    if (!returnType.includes("TErrTuple")) {
        return {
            error: "Effectful function must return TErrTuple<Data>. " +
            `Found: ${returnType}. ` +
            "You might want to read .opencode/agent/function-builder.md"
        };
    }
